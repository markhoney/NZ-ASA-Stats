...
/*
<VirtualHost *:80>
    ServerName asa.sbh.nz
    DocumentRoot "/var/www/html/asa"
...
$GLOBALS['sbh'] = ['M. Honeychurch', 'M. Hanna', 'L. Taylor', 'D. Ryan', 'C. Gold', 'S. McAuliffe', 'R. Seddon-Smith', 'M. Edmonds', 'C. Atkinson', 'S. Clark', 'M. Willey', 'T. Atkin', 'L. Oldfield', 'K. Honeychurch', 'M. Taylor', 'C. Morgan', 'N. Grange', 'P. Muir', 'A. Gilbey', 'M. Coffey', 'M. Taylor', 'R. Shillito', 'C. Suen', 'L. Marron', 'K. Perrott', 'T. Linney', 'B. Lennox', 'S. Sinclair', 'K. Hester', 'S. Taylor'];
$GLOBALS['msof'] = ['M. Honeychurch', 'M. Hanna', 'L. Taylor', 'D. Ryan', 'C. Atkinson', 'M. Willey', 'L. Oldfield', 'K. Perrott', 'A. Chang'];
...
$GLOBALS['sql']['join']['appeals'] = " LEFT JOIN appeals ON complaints.id = appeals.complaints_id ";
$GLOBALS['sql']['count'] = "SELECT COUNT(DISTINCT complaints.id) AS complaints ";
$GLOBALS['sql']['tables']['complainants'] = "FROM complaints_complainants, complainants, complaints_decisions, decisions, complaints " . $GLOBALS['sql']['join']['appeals'];
$GLOBALS['sql']['tables']['advertisers'] = "FROM complaints_advertisers, advertisers, complaints_decisions, decisions, complaints " . $GLOBALS['sql']['join']['appeals'];
$GLOBALS['sql']['tables']['media'] = "FROM complaints_media, media, complaints_decisions, decisions, complaints " . $GLOBALS['sql']['join']['appeals'];
$GLOBALS['sql']['tables']['products'] = "FROM complaints_decisions, decisions, complaints_products, products, complaints " . $GLOBALS['sql']['join']['appeals'];
$GLOBALS['sql']['tables']['years'] = "FROM complaints_decisions, decisions, complaints " . $GLOBALS['sql']['join']['appeals'];
//$GLOBALS['sql']['tables']['codes'] = "FROM codes_clauses, codes, complaints_codes_clauses, complaints_decisions, decisions, complaints " . $GLOBALS['sql']['join']['appeals'];
$GLOBALS['sql']['tables']['codes'] = "FROM complaints_decisions, decisions, (SELECT complaints.*, codes.name FROM codes_clauses, codes, complaints_codes_clauses, complaints WHERE codes_clauses.codes_id = codes.id AND codes_clauses.id = complaints_codes_clauses.codes_clauses_id AND complaints_codes_clauses.complaints_id = complaints.id GROUP BY complaints_codes_clauses.complaints_id, codes.name) AS complaints LEFT JOIN appeals ON complaints.id = appeals.complaints_id ";
$GLOBALS['sql']['tables']['code'] = "FROM complaints_decisions, decisions, complaints " . $GLOBALS['sql']['join']['appeals'];
$GLOBALS['sql']['tables']['clauses'] = "FROM codes_clauses, codes, complaints_codes_clauses, complaints_decisions, decisions, complaints " . $GLOBALS['sql']['join']['appeals'];
$GLOBALS['sql']['link']['complainants'] = "WHERE complaints_complainants.complainants_id = complainants.id AND complaints_complainants.complaints_id = complaints.id AND complaints_decisions.complaints_id = complaints.id ";
$GLOBALS['sql']['link']['advertisers'] = "WHERE complaints_advertisers.advertisers_id = advertisers.id AND complaints_advertisers.complaints_id = complaints.id AND complaints_decisions.complaints_id = complaints.id ";
$GLOBALS['sql']['link']['media'] = "WHERE complaints_media.media_id = media.id AND complaints_media.complaints_id = complaints.id AND complaints_decisions.complaints_id = complaints.id ";
$GLOBALS['sql']['link']['products'] = "WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id AND complaints_products.complaints_id = complaints.id AND complaints_products.products_id = products.id";
$GLOBALS['sql']['link']['years'] = "WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id ";
//$GLOBALS['sql']['link']['codes'] = "WHERE codes_clauses.codes_id = codes.id AND codes_clauses.id = complaints_codes_clauses.clauses_id AND complaints_codes_clauses.complaints_id = complaints.id AND complaints_decisions.complaints_id = complaints.id ";
$GLOBALS['sql']['link']['codes'] = "WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id ";
$GLOBALS['sql']['link']['clauses'] = "WHERE codes_clauses.codes_id = codes.id AND codes_clauses.id = complaints_codes_clauses.codes_clauses_id AND complaints_codes_clauses.complaints_id = complaints.id AND complaints_decisions.complaints_id = complaints.id ";
$GLOBALS['sql']['extra']['codes'] = "(SELECT DISTINCT complaints_id FROM complaints_codes_clauses WHERE codes_clauses_id IN (SELECT id FROM codes_clauses WHERE codes_id = (SELECT id FROM codes WHERE name = ?)))";
$GLOBALS['sql']['successes'] = "((decisions.success = 1 AND decisions.name LIKE '%Settled%') OR (decisions.success = 1 AND IFNULL(appeals.success, 0) = 0) OR (decisions.success = 0 AND IFNULL(appeals.success, 0) = 1))";
$GLOBALS['sql']['stats']['total'] = "COUNT(DISTINCT complaints.id) AS total ";
#$GLOBALS['sql']['stats']['winloss'] = " ((decisions.success = 1 AND decisions.name LIKE '%Settled%') OR (decisions.success = 1 AND IFNULL(appeals.success, 0) = 0) OR (decisions.success = 0 AND IFNULL(appeals.success, 0) = 1)) ";
#$GLOBALS['sql']['stats']['winloss'] = " ((decisions.success = 1 AND decisions.name LIKE '%Settled%') OR (decisions.success = 1 AND IFNULL(appeals.success, 0) = 0) OR (decisions.success = 0 AND IFNULL(appeals.success, 0) = 1)) ";
#$GLOBALS['sql']['stats']['win'] = $GLOBALS['sql']['stats']['winloss'] . " AS win ";
#$GLOBALS['sql']['stats']['loss'] = "(1 - " . $GLOBALS['sql']['stats']['winloss'] . " AS loss ";
$GLOBALS['sql']['stats']['draw'] = "(decisions.success IS NULL) AS draw ";
$GLOBALS['sql']['stats']['win'] = "(decisions.success = 1) AS win ";
$GLOBALS['sql']['stats']['loss'] = "(decisions.success = 0) AS loss ";
...
$GLOBALS['sql']['multiples'] = "(SELECT GROUP_CONCAT(name SEPARATOR '|') FROM codes WHERE id IN (SELECT DISTINCT codes_id FROM codes_clauses WHERE id IN (SELECT codes_clauses_id FROM complaints_codes_clauses WHERE complaints_id = complaints.id))) AS codes, (SELECT GROUP_CONCAT(CONCAT(codes.name, ', ', codes_clauses.name) SEPARATOR '|') FROM codes_clauses LEFT JOIN codes ON codes_clauses.codes_id = codes.id WHERE codes_clauses.id IN (SELECT codes_clauses_id FROM complaints_codes_clauses WHERE complaints_id = complaints.id)) AS codes_clauses, (SELECT GROUP_CONCAT(name SEPARATOR '|') FROM media WHERE media.id IN (SELECT media_id FROM complaints_media WHERE complaints_id = complaints.id)) AS media, (SELECT GROUP_CONCAT(name SEPARATOR '|') FROM complainants WHERE complainants.id IN (SELECT complainants_id FROM complaints_complainants WHERE complaints_id = complaints.id)) AS complainants, (SELECT GROUP_CONCAT(name SEPARATOR '|') FROM advertisers WHERE advertisers.id IN (SELECT advertisers_id FROM complaints_advertisers WHERE complaints_id = complaints.id)) AS advertisers ";
$GLOBALS['sql']['complaint'] = "complaints.id as id, CONCAT(SUBSTR(complaints.id, 1, 2), '/', SUBSTR(complaints.id, 3)) AS idslash, year, advertisement, IFNULL(meeting, '[None]') AS meeting, url, decisions.name AS decision, decisions.name, decisions.success, appeals.success as appealsuccess, appeals.id as appealid, CONCAT(SUBSTR(appeals.id, 1, 2), '/', SUBSTR(appeals.id, 3)) AS appealidslash, IFNULL((SELECT name FROM products WHERE id = (SELECT products_id FROM complaints_products WHERE complaints_id = complaints.id)), '[None]') AS product";
...
$GLOBALS['sql']['list']['complaints']['company'] = "(SELECT complaints_id FROM complaints_advertisers WHERE advertisers_id = (SELECT id FROM advertisers WHERE name = ?))";
...
//$GLOBALS['sql']['list']['complaints']['clause'] = "(SELECT complaints_id FROM complaints_codes_clauses WHERE codes_clauses_id = (SELECT id FROM codes_clauses WHERE name = ?))";

$GLOBALS['typedata'] = ["complaints" => ["complaint", "id"], "complainants" => ["complainant", "name"], "advertisers" => ["company", "name"], "media" => ["medium", "name"], "products" => ["product", "name"], "codes" => ["code", "name"], "codes_clauses" => ["clause", "name"], "badges" => ["badge", "name"], "years" => ["year", "name"], "appeals" => ["appeal", "name"]];
 ...
 <body>
  <div id="content" class="container">
   ...
   <div class="alert alert-danger" role="alert">
    <strong>Warning!</strong> This website is currently undergoing major reconstruction. Please apply <a href="http://skeptics.nz/" target="_blank">skepticism</a> to any numbers you see, as many of them are just plain wrong!
   </div>
...
function getData($query, $vars = []) {
 if (__FILE__ == "/mnt/media/Unsorted/ASA/asa.sbh.nz/html/index.php") {
  /*print '<pre>';
  print $query . "\n";
  print_r($vars);
  print '</pre>';*/
	...
	print ');</script>';
	print '<script>console.log(';
	print json_encode(print_r($vars, true));
	print ');</script>';
  //exit();
 }
 ...
 if (__FILE__ == "/mnt/media/Unsorted/ASA/asa.sbh.nz/html/index.php") {
  ...
  //print '</pre>';
	//print '<script>console.log("';
  //print json_encode($stmt->debugDumpParams());
	//print '");</script>';
 }
 if ($stmt->rowCount() > 0) {
...
function badges($complainant) {
 ...
 if ($totals['total'] >= 1) {
  ...
  $idquery = "(SELECT complaints.id " . $GLOBALS['sql']['tables']['complainants'] . $GLOBALS['sql']['link']['complainants'] . " AND ((decisions.success = 1 AND decisions.name LIKE '%Settled%') OR (decisions.success = 1 AND IFNULL(appeals.success, 0) = 0) OR (decisions.success = 0 AND IFNULL(appeals.success, 0) = 1)) = 1 AND complainants.name = ?)";
  ...
  foreach ($words as $word => $values) {
   $badges[$word] = (getData("SELECT COUNT(DISTINCT complaints.id) as win FROM docs WHERE complaints_id IN " . $idquery . " AND contents REGEXP ?", [$complainant, implode("|", $values)])[0]['win'] >= 1 ? 1 : 0);
  }
  $badges['essay'] = (getData($query . "AND docwords >= 10000 " . $GLOBALS['sql']['filter'], [$complainant])[0]['total'] >= 1 ? 1 : 0);
  $clauses = getData("SELECT COUNT(DISTINCT complaints.id) AS codes_clauses " . $GLOBALS['sql']['tables']['complainants'] .", complaints_codes_clauses, codes_clauses " . $GLOBALS['sql']['link']['complainants'] . "AND complaints_codes_clauses.complaints_id = complaints.id AND codes_clauses.id = complaints_codes_clauses.codes_clauses_id " . $GLOBALS['sql']['filter'] . "GROUP BY complaints.id ORDER BY codes_clauses DESC LIMIT 1", [$complainant])[0]['codes_clauses'];
  ...
  $codecount = 0;
  $codelist = getData("SELECT DISTINCT codes.name AS code FROM complaints_complainants, complaints_decisions, complainants, complaints, complaints_decisions, decisions, complaints_codes_clauses, codes_clauses, codes WHERE complaints_complainants.complainants_id = complainants.id AND complaints_complainants.complaints_id = complaints.id AND complaints_decisions.complaints_id = complaints.id AND complaints_codes_clauses.complaints_id = complaints.id AND codes_clauses.id = complaints_codes_clauses.codes_clauses_id AND codes.id = codes_clauses.codes_id AND decisions.success = 1 AND complainants.name = ?", [$complainant]);
  if ($codelist) {
  ...
  $mediacount = 0;
  if ($medialist = getData("SELECT DISTINCT media.name AS medium FROM complaints_complainants, complaints_decisions, complainants, complaints, complaints_decisions, decisions, complaints_media, media WHERE complaints_complainants.complainants_id = complainants.id AND complaints_complainants.complaints_id = complaints.id AND complaints_decisions.complaints_id = complaints.id AND complaints_media.complaints_id = complaints.id AND media.id = complaints_media.media_id AND decisions.success = 1 AND complainants.name = ?", [$complainant])) {
...
function complaintsTable(&$complaints, $sort = "sorter", $complainant = False, $company = False) {
 if (is_array($complaints)) {
  ...
  foreach ($complaints as $id => $complaint) {
   $table .= '<tr class="' . $GLOBALS['results'][$complaint['result']] . '"><td><a href="/complaint/' . $id . '">' . $complaint['idslash'] . '</a></td>' . ($complainant ? '<td>' . implode(", ", linkList('complainant', $complaint['complainants'])) . '</td>' : '') . ($company ? '<td>' . implode(", ", linkList('company', $complaint['advertisers'])) . '</td>' : '') . '<td><a href="/complaint/' . $id . '">' . $complaint['advertisement'] . '</a></td><td>' . $complaint['product'] . '</td><td>' . $complaint['meeting'] . '</td></tr>' . "\n";
  }
...
function getComplaints($filter, $array = [], $win = False, $upheld = False) {
 if ($complaints = getData("SELECT complaints.id as id, CONCAT(SUBSTR(complaints.id, 1, 2), '/', SUBSTR(complaints.id, 3)) AS idslash, advertisement, IFNULL(meeting, '[None]') as meeting, IFNULL((SELECT name FROM products WHERE id = (SELECT products_id FROM complaints_products WHERE complaints_id = complaints.id)), '[None]') AS product, " . $GLOBALS['sql']['stats']['win'] . ", " . $GLOBALS['sql']['stats']['draw'] . " FROM complaints LEFT JOIN appeals ON complaints.id = appeals.complaints_id LEFT JOIN complaints_decisions ON complaints.id = complaints_decisions.complaints_id LEFT JOIN decisions ON complaints_decisions.decisions_id = decisions.id LEFT JOIN complaints_products ON complaints.id = complaints_products.complaints_id LEFT JOIN products ON complaints_products.products_id = products.id WHERE complaints.id IN " . $filter . ($win ? " AND decisions.success = 1 AND IFNULL(appeals.success, 0) = 0 " : "") . ($upheld ? " AND decisions.name LIKE '%Upheld%' " : "") . " ORDER BY complaints.meeting DESC, complaints.id DESC;", $array)) {
...
function getComplaint($id) {
 #if ($complaint = getData("SELECT " . $GLOBALS['sql']['complaint'] . ', ' . $GLOBALS['sql']['stats']['win'] . ', ' . $GLOBALS['sql']['stats']['draw'] . ', ' . $GLOBALS['sql']['multiples'] . "FROM complaints_decisions, decisions, complaints LEFT JOIN appeals ON complaints.id = appeals.complaints_id WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id AND complaints.id = ?", [$id])) {
 if ($complaint = getData("SELECT complaints.id as id, CONCAT(SUBSTR(complaints.id, 1, 2), '/', SUBSTR(complaints.id, 3)) AS idslash, year, advertisement, IFNULL(meeting, '[None]') AS meeting, complaints.url, docs.html, decisions.name AS decision, decisions.name, decisions.success, IFNULL((SELECT name FROM products WHERE id = (SELECT products_id FROM complaints_products WHERE complaints_id = complaints.id)), '[None]') AS product, (decisions.success = 1) AS win, (decisions.success IS NULL) AS draw, IFNULL((SELECT name FROM codes WHERE id = (SELECT codes_id FROM complaints_codes WHERE complaints_id = complaints.id)), '[None]') AS codes, (SELECT GROUP_CONCAT(name SEPARATOR '|') FROM media WHERE media.id IN (SELECT media_id FROM complaints_media WHERE complaints_id = complaints.id)) AS media, (SELECT GROUP_CONCAT(name SEPARATOR '|') FROM complainants WHERE complainants.id IN (SELECT complainants_id FROM complaints_complainants WHERE complaints_id = complaints.id)) AS complainants, (SELECT GROUP_CONCAT(name SEPARATOR '|') FROM advertisers WHERE advertisers.id IN (SELECT advertisers_id FROM complaints_advertisers WHERE complaints_id = complaints.id)) AS advertisers FROM complaints LEFT JOIN complaints_decisions ON complaints.id = complaints_decisions.complaints_id LEFT JOIN decisions ON complaints_decisions.decisions_id = decisions.id LEFT JOIN docs ON complaints.id = docs.complaints_id AND complaints.source = docs.complaints_source WHERE complaints_decisions.decisions_id = decisions.id AND complaints.id = ?", [$id])) {
  $complaint = $complaint[0];
  foreach (["codes_clauses", "media", "complainants", "advertisers"] as $multi) {
	 if (isset($complaint[$multi])) {
...
function getComplaintsDetails($filter, $array = [], $win = False, $upheld = False) {
 if ($complaints = getData("SELECT " . $GLOBALS['sql']['complaint'] . ', ' . $GLOBALS['sql']['stats']['win'] . ', ' . $GLOBALS['sql']['stats']['draw'] . ', ' . $GLOBALS['sql']['multiples'] . "FROM complaints_decisions, decisions, complaints LEFT JOIN appeals ON complaints.id = appeals.complaints_id WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id " . ($win ? " AND decisions.success = 1 AND IFNULL(appeals.success, 0) = 0 " : "") . ($upheld ? " AND decisions.name LIKE '%Upheld%' " : "") . " AND complaints.id IN " . $filter . " ORDER BY complaints.meeting DESC, complaints.id DESC;", $array)) {
  foreach ($complaints as &$complaint) {
   foreach (["codes", "codes_clauses", "media", "complainants", "advertisers"] as $multi) {
    $complaint[$multi] = explode("|", $complaint[$multi]);
   }
   complaintResult($complaint);
   complaintLinks($complaint);
...
function advertisersData($limit = 0) {
 #$data = getData($GLOBALS['sql']['stats']['all'] . ", advertisers.name " . $GLOBALS['sql']['tables']['advertisers'] . $GLOBALS['sql']['link']['advertisers'] . " AND advertisers.name != 'Advertiser unknown' GROUP BY advertisers.name ORDER BY win DESC, total ASC, advertisers.name ASC" . ($limit ? " LIMIT " . $limit : "") . ";");
 $data = getData("SELECT COUNT(*) AS total, SUM(decisions.success = 1) AS win, SUM(decisions.success = 0) AS loss, SUM(decisions.success IS NULL) AS draw, advertisers.name FROM advertisers, complaints_advertisers LEFT JOIN complaints_decisions ON complaints_advertisers.complaints_id = complaints_decisions.complaints_id LEFT JOIN decisions ON complaints_decisions.decisions_id = decisions.id WHERE complaints_advertisers.advertisers_id = advertisers.id AND advertisers.name NOT IN ('Other', 'Others') GROUP BY advertisers.name ORDER BY win DESC, total ASC, advertisers.name ASC" . ($limit ? " LIMIT " . $limit : "") . ";");
 calcStats($data);
 return codekey($data, "name");
}
...
function complainantsData($limit = 0) {
 ...
 $data = getData("SELECT COUNT(*) AS total, SUM(decisions.success = 1) AS win, SUM(decisions.success = 0) AS loss, SUM(decisions.success IS NULL) AS draw, complainants.name FROM complaints_complainants, complainants, complaints_decisions, decisions WHERE complaints_complainants.complainants_id = complainants.id AND complaints_complainants.complaints_id = complaints_decisions.complaints_id AND complaints_decisions.decisions_id = decisions.id AND complainants.name NOT IN ('Other', 'Others') GROUP BY complainants.name ORDER BY win DESC, total ASC, complainants.name ASC" . ($limit ? " LIMIT " . $limit : "") . ";");
 calcStats($data);
 return codekey($data, "name");
}
...
function yearsData() {
 ...
 $data = getData($GLOBALS['sql']['stats']['all'] . ", year as name FROM complaints LEFT JOIN complaints_decisions ON complaints.id = complaints_decisions.complaints_id LEFT JOIN decisions ON complaints_decisions.decisions_id = decisions.id  GROUP BY year ORDER BY year DESC;");
 calcStats($data);
...
function clausesData() {
 $data = getData($GLOBALS['sql']['stats']['all'] . ", CONCAT(codes.name, ', ', codes_clauses.name) AS name " . $GLOBALS['sql']['tables']['clauses'] . $GLOBALS['sql']['link']['clauses'] . "  GROUP BY codes_clauses.id ORDER BY total DESC;");
 calcStats($data);
 return codekey($data, "name");
}
...
if (!empty($GLOBALS['get'])) {
 if (($term = getQuery(['search'])) !== null) {
  if (!$term) {
   ...
   if (array_key_exists('upheld', $_GET)) {
    $output["complaints"] = getComplaints("(SELECT DISTINCT complaints_id FROM docs WHERE contents LIKE ?)", ['%' . $term . '%'], True, True);
   } elseif (array_key_exists('success', $_GET)) {
    $output["complaints"] = getComplaints("(SELECT DISTINCT complaints_id FROM docs WHERE contents LIKE ?)", ['%' . $term . '%'], True);  
   } else {
    $output["complaints"] = getComplaints("(SELECT DISTINCT complaints_id FROM docs WHERE contents LIKE ?)", ['%' . $term . '%']);
   }
 ...
 } elseif (($term = getQuery(['company', 'advertisers'])) !== null) {
  if (!$term) {
   $data = advertisersData();
   if ($GLOBALS['filetype'] == 'html') {
    printheader('ASA Advertiser Rankings');
    print '<h1>ASA Advertisers</h1>' . rankingTable($data, "company", "http://schema.org/Organization", False);
   } else {
    api($data, 'advertisers');
   }
  } else {
   if ($output = getData($GLOBALS['sql']['stats']['all'] . $GLOBALS['sql']['tables']['advertisers'] . $GLOBALS['sql']['link']['advertisers'] . " AND advertisers.name = ?", [$term])[0]) {
    calcStat($output);
    $output["complaints"] = getComplaintsDetails("(SELECT complaints_id FROM complaints_advertisers WHERE advertisers_id = (SELECT id FROM advertisers WHERE name = ?))", [$term]);
    if ($GLOBALS['filetype'] == 'html') {
  ...
  if (!$term) {
   ...
   if ($GLOBALS['filetype'] == 'html') {
    ...
    print '<h1>ASA Complainants</h1>' . rankingTable($data, "complainant", "http://schema.org/Person", False);
   } else {
  ...
  if (!$term) {
   ...
   $output = getData($GLOBALS['sql']['stats']['all'] . $GLOBALS['sql']['tables']['code'] . $GLOBALS['sql']['link']['codes'] . " AND complaints.id IN " . $GLOBALS['sql']['extra']['codes'], [$term])[0];
   if ($output) {
    calcStat($output);
  ...
  if (!$term) {
   ...
   $clause = explode(", ", $term);
   $output = getData($GLOBALS['sql']['stats']['all'] . $GLOBALS['sql']['tables']['clauses'] . $GLOBALS['sql']['link']['clauses'] . " AND codes.name = ? AND codes_clauses.name = ?", $clause)[0];
   if ($output) {
    calcStat($output);
    $output["complaints"] = getComplaints("(SELECT DISTINCT complaints_id FROM complaints_codes_clauses WHERE codes_clauses_id = (SELECT id FROM codes_clauses WHERE codes_id = (SELECT id FROM codes WHERE name = ?) AND name = ?))", $clause);
    if ($GLOBALS['filetype'] == 'html') {
  ...
  if (!$term) {
   ...
   $output["complaints"] = getComplaintsDetails("(SELECT id FROM complaints WHERE meeting > (NOW() - INTERVAL 6 MONTH) AND meeting < (NOW() + INTERVAL 1 DAY) AND id IN (SELECT complaints_id FROM complaints_complainants WHERE complainants_id IN (SELECT id FROM complainants WHERE complainants.name IN ('" . implode("', '", $GLOBALS['sbh'])  . "'))) ORDER BY meeting DESC)");
   ...
   $output["complaints"] = getComplaintsDetails("(SELECT id FROM complaints WHERE id IN (SELECT complaints_id FROM complaints_complainants WHERE complainants_id IN (SELECT id FROM complainants WHERE complainants.name IN ('" . implode("', '", $GLOBALS['sbh'])  . "'))) ORDER BY meeting DESC)");
   if ($GLOBALS['filetype'] == 'html') {
 ...
 } elseif (($term = getQuery(['msof'])) !== null) {
  $output = getData($GLOBALS['sql']['stats']['all'] . $GLOBALS['sql']['tables']['complainants'] . $GLOBALS['sql']['link']['complainants'] . " AND complaints.id IN (SELECT complaints_id FROM docs WHERE contents LIKE '%fluori%') AND complainants.name IN ('" . implode("', '", $GLOBALS['sbh'])  . "')")[0];
  calcStat($output);
  $output["complaints"] = getComplaintsDetails("(SELECT id FROM complaints WHERE id IN (SELECT complaints_id FROM complaints_complainants WHERE complaints.id IN (SELECT complaints_id FROM docs WHERE contents LIKE '%fluori%') AND complainants_id IN (SELECT id FROM complainants WHERE complainants.name IN ('" . implode("', '", $GLOBALS['msof'])  . "'))) ORDER BY meeting DESC)");
  if ($GLOBALS['filetype'] == 'html') {
   printheader('MSoF Member Complaints');
   print "<h2>MSoF Member Complaints</h2>";
   //print statsList($output);
   print statsList($output);
   print complaintsTable($output["complaints"], "filter", True, True);
  } else {
  ...
  $data = ["unsuccessfuloverturned" => getComplaintsDetails("(SELECT complaints.id FROM complaints, complaints_decisions, decisions, appeals WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id AND complaints.id = appeals.complaints_id AND decisions.success = 0 AND appeals.success = 1)")];
  $data["successfuldefended"] = getComplaintsDetails("(SELECT complaints.id FROM complaints, complaints_decisions, decisions, appeals WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id AND complaints.id = appeals.complaints_id AND decisions.success = 1 AND appeals.success = 0)");
  $data["successfuloverturned"] = getComplaintsDetails("(SELECT complaints.id FROM complaints, complaints_decisions, decisions, appeals WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id AND complaints.id = appeals.complaints_id AND decisions.success = 1 AND appeals.success = 1)");
  $data["unsuccessfuldefended"] = getComplaintsDetails("(SELECT complaints.id FROM complaints, complaints_decisions, decisions, appeals WHERE complaints_decisions.complaints_id = complaints.id AND complaints_decisions.decisions_id = decisions.id AND complaints.id = appeals.complaints_id AND decisions.success = 0 AND appeals.success = 0)");
  if ($GLOBALS['filetype'] == 'html') {
   print "<h1>Appeals</h1>" . "\n";
  ...
  if (!$term or (strpos($term, ",") !== FALSE) or $term == 'details') {
   ...
   if ($complaint) {
    if ($GLOBALS['filetype'] == 'html') {
     ...
     print "<h2>" . $complaint['advertisement'] . "</h2>" . "\n";
     ...
     foreach ($complaint['advertisers'] as &$company) {
      $company = '<a href="/company/' . urlencode($company) . '">' . $company . '</a>';
     }
     print '<dt>advertisers</dt><dd>' . implode('<br />', $complaint['advertisers']) . '</dd>';
     ...
     foreach ($complaint['codes_clauses'] as &$clause) {
     ...
     print '<dt>Clauses</dt><dd>' . implode('<br />', $complaint['codes_clauses']) . '</dd>';
     ...
     print '</dl>';
     print '</div></div>';
     $html = make_clickable(iconv('UTF-8', 'ASCII//TRANSLIT', file_get_contents('/mnt/media/Unsorted/ASA/files/' . $complaint['html'])));
 ...
 $data["advertisers"] = advertisersData($top);
 $data["products"] = productsData();
 $data["years"] = yearsData();
 $data["media"] = mediaData();
 $data["codes"] = codesData();
 $data["complaints"] = getComplaintsDetails("(SELECT id FROM complaints WHERE meeting > (NOW() - INTERVAL 2 MONTH) AND meeting < (NOW() + INTERVAL 1 DAY) ORDER BY meeting DESC)");
 if ($GLOBALS['filetype'] == 'html') {
  printheader('ASA Complaints');
  ...
  print "<h2>Bottom " . $top . ' <a href="/company">Advertisers</a></h2>';
  print rankingTable($data["advertisers"], "company", "http://schema.org/Organization", False);
  print '</div></div>';
  print '<div class="row"><div class="col-md-6">';
